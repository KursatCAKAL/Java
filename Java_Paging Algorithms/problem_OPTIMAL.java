...
public class problem_OPTIMAL {
    ...
    problem_OPTIMAL(int gelenCerceve, int[] gelenReferansDizi)
    {
    ...
    public int optiomal_method(){
        ...
        for (int i=0;i<referansDizisi.length;i++)
        {
            ...
            else//değer yok ise hata değerini arttır. OPTIMAL kuralına göre elemanı cerceve dizisine yerleştir.
            {
                sayfa_hatasi++;
                ...
                //bu çıkardığın değer yerine yeni eklemen gerekeni ekle
                {
                    ...
                    {
                        int sayac=0;
                        //bulunulan yerden sayac degeri kadar ilerde kullanıldığını buluyoruz.
                        for (int c=i;c<referansDizisi.length;c++)//dolaştığın cerceve değeri için gezdiğin rakamlara eşit olmadığı sürece sayacı arttır.son eşit olduğu yeri de işaretle
                        {
                            if(cerceveDizisi[j]!=referansDizisi[c])//bulunmadığı sürece say
                            {
                                sayac++;
                                if(c==referansDizisi.length-1)
                                //eğer olurda değer o indisten sonuncu indise kadar hiç bulunmaz ise dizinin sonuna kadar o degerin olmadığını ifade et
